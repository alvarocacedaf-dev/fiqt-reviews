-- Actualiza los cinco niveles de la ruta de recompensas a 2, 5, 7, 10 y 18.
...
begin
  ...
  v_limit := case
    when public.is_admin() or v_reviews >= 18 then 100000
    when v_reviews >= 10 then 2
    when v_reviews >= 5 then 1
    else 0
  end;

  if p_selected then
    if v_limit = 0 then raise exception 'Necesitas 5 reseñas aprobadas para seleccionar un curso.'; end if;
    if not exists (
...
begin
  if v_user_id is null then raise exception 'Debes iniciar sesión.'; end if;
  if not public.has_worksheet_access(v_user_id) then raise exception 'Necesitas 7 reseñas aprobadas para usar Planchas.'; end if;
  if exists (
