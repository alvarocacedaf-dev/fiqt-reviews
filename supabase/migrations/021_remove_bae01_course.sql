-- Retira BAE01 del catálogo de cursos.
...
begin
  ...
  where upper(trim(course.code)) = 'BAE01'
  limit 1;

  if v_course_id is null then
    raise notice 'El curso BAE01 ya no existe; no se realizó ningún cambio.';
    return;
  end if;
  ...
  if exists (
    ...
    raise exception
      'BAE01 tiene planchas administrativas. Elimínalas desde el panel antes de retirar el curso.';
  end if;
