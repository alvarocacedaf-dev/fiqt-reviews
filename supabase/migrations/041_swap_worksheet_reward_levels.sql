-- Intercambia las recompensas de 5 y 7 reseñas aprobadas:
-- 5 reseñas: comunidad de planchas.
-- 7 reseñas: selección y descarga de un curso de la administración.
...
begin
  if v_user_id is null then raise exception 'Debes iniciar sesión.'; end if;
  if not public.has_worksheet_access(v_user_id) then raise exception 'Necesitas 5 reseñas aprobadas para usar Planchas.'; end if;
  if exists (
