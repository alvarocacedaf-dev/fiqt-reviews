...
function safeFileName(name: string) {
  ...
  const base = name.slice(0, extension ? -extension.length : undefined)
    ...
    .replace(/[^a-zA-Z0-9_-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 100) || 'material';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (fileSize > MAX_FILE_SIZE) return NextResponse.json({ error: `“${fileName}” supera el límite de 25 MB.` }, { status: 400 });
